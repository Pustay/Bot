...
async def mes_start(message: types.Message):
    ...
    await message.answer(f'{name}, привет! Сегодня сыграем с тобой в конфеты! Для начала игры против бота, введи команду /new_game. '
                        f'Для начала игры против игрока, введи команду /duel и через пробел укажи id игрока, против которого хотите сыграть.'
                        f'Для настройки конфет введи команду /set и через пробел укажи количество конфет.')
    print(message.from_user.id)

@dp.message_handler(commands=['new_game', 'новая игра'])
async def mes_new_game(message: types.Message):
    ...
    if first:
        await message.answer(f'Игра началась. По жребию первым ходит {name}. Бери конфеты...')
    else:
...
async def mes_duel(message: types.Message):
    ...
    global first
    global current
    duel.append(int(message.from_id))
    duel.append(int(message.text.split()[1]))
    name = message.from_user.full_name
    total = max_count
    first = random.randint(0,1)
    if first:
        await dp.bot_send_message(duel[0], 'Первый ход за тобой, бери конфеты...')
        await dp.bot_send_message(duel[1], 'Первый ход за твоим противником! Жди своего хода.')
    else:
        await dp.bot_send_message(duel[1], 'Первый ход за тобой, бери конфеты...')
        await dp.bot_send_message(duel[0], 'Первый ход за твоим противником! Жди своего хода.')
    current = duel[0] if first else duel[1]
    new_game = True

@dp.message_handler(commands=['set', 'поставить' ])
            ...
            if new_game:
                if message.text.isdigit() and 0 < int(message.text) < 29:
                    ...
                    if total <= 0:
                        ...
                        await message.answer(f'{name} взял(а) {message.text} конфет. '
                                            f'На столе осталось {total}')
                        await dp.bot.send_message(enemy_id(), f'Теперь твой ход бери конфеты! На столе осталось {total}')
                        switch_players()
                else:
                    await message.answer(f'{name}, надо указать ЧИСЛО от 1 до 28!')
